use crate::{
    ...
    errors::ServiceError,
    models::user::User,
    schema::{task_status_for_user, task_types_for_user},
};
...
impl StatusTable {
    ...
    pub fn create_status_by_name(
        value: &str,
        user_id: uuid::Uuid,
        conn: &Connection,
    ) -> Result<StatusTable, ServiceError> {
        Ok(
            diesel::insert_into(task_status_for_user::dsl::task_status_for_user)
                .values((
                    task_status_for_user::dsl::user_id.eq(user_id),
                    task_status_for_user::dsl::status_name.eq(value),
                ))
                .get_result::<StatusTable>(conn)?,
        )
    }
...
impl TypeTable {
    ...
    pub fn create_type_by_name(
        value: &str,
        user_id: uuid::Uuid,
        conn: &Connection,
    ) -> Result<TypeTable, ServiceError> {
        Ok(
            diesel::insert_into(task_types_for_user::dsl::task_types_for_user)
                .values((
                    task_types_for_user::dsl::user_id.eq(user_id),
                    task_types_for_user::dsl::type_name.eq(value),
                ))
                .get_result::<TypeTable>(conn)?,
        )
    }
